...
foreach ($providers as $provider){
	echo CHtml::link('<img src="'.$path.$provider.'.png" alt="'.$provider.'" title="'.$provider.'" />',array('/site/authenticatewith/provider/'.$provider));
	}
?>
...
foreach($openid_servers as $image=>$server){
	echo CHtml::link('<img src="'.$path.$image.'.png" alt="'.$server.'" title="'.$server.'"/>',array('/site/authenticatewith?openid='.$server));
}
